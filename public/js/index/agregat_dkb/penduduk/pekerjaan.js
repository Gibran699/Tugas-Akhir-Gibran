(function ($) {
    ...
    function fetchData() {
        var formData = new FormData(
            document.getElementById("formPendudukPekerjaan")
        );
        const job = [
        ...
        $.ajax({
            url: $("#formPendudukPekerjaan").attr("action"),
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                ...
                $("#tahunSemester").text(
                    `Penduduk Pekerjaan Tahun ${tahun} - Semester ${semester}`
                );
            },
            error: function (xhr) {
    ...
    function setTableKecamatan(response, job) {
        ...
        let dataSet = response.dataPerkecamatan.map((item) => {
            // Start with the fixed columns (kecamatan_nama)
        ...
        // Initialize DataTable with FixedHeader and FixedColumns
