(function($) {
    ...
    function fetchData() {
        var formData = new FormData(document.getElementById('formSearchKepalaKeluargaJenisKelamin'));
        $.ajax({
            url: $('#formSearchKepalaKeluargaJenisKelamin').attr('action'),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    var pieChart = function(response) {
        if (jQuery('#chartKepalaKeluargaJenisKelamin').length > 0) {
            const dataKeseluruhan = response.dataKeseluruhan;
            if (dataKeseluruhan) {
                const pie_chart = document.getElementById("chartKepalaKeluargaJenisKelamin").getContext('2d');
                new Chart(pie_chart, {
                    type: 'pie',
                    data: {
                        datasets: [{
