(function ($) {
    ...
    function fetchData() {
        var formData = new FormData(
            document.getElementById("formSearchPendudukUsiaMudaProduktif")
        );
        const atributTable = [
            'usia_muda',
        'usia_produktif',
        ...
        $.ajax({
            url: $("#formSearchPendudukUsiaMudaProduktif").attr("action"),
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                ...
                $("#tahunSemester").text(
                    `Penduduk Usia Muda, Produktif & Tua Tahun ${tahun} - Semester ${semester}`
                );
            },
            error: function (xhr) {
    ...
    function setTableKelurahan(response, atributTable) {
        ...
        let dataSet = response.dataPerkelurahan.map((item) => {
            ...
            atributTable.forEach((religionKey) => {
        ...
        atributTable.forEach((religionKey) => {
            columns.push({ title: religionKey.toUpperCase() }); // Add the atributTable key as the column title
        });
