(function ($) {
    ...
    function fetchData() {
        var formData = new FormData(document.getElementById('formSearchGolonganDarahKelompokUmur'));
        const blood = [
            'a_lk',
            'a_pr',
            'a_jml',
        ...
        $.ajax({
            url: $('#formSearchGolonganDarahKelompokUmur').attr('action'),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (response) {
                ...
                $("#tahunSemester").text(`Penduduk Golongan Darah Kelompok Umur Tahun ${tahun} - Semester ${semester}`);
            },
            error: function (xhr) {
    ...
    function setTableKelurahan(response, blood) {
        ...
        blood.forEach(religionKey => {
            columns.push({ title: religionKey.toUpperCase() }); // Add the blood key as the column title
        });
    ...
    function setTableSum(response, blood) {
        ...
        summedData.forEach(data => {
            blood.forEach(bloodKey => {
                ...
                const bloodName = bloodKey.replace(/_/g, ' ').toUpperCase();
